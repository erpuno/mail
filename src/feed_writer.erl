...
handle_call({cached_feed,FId,Page},From,State) ->
    ...
    Reply = case State#state.cached_feed of
                 undefined -> store:entries_in_feed(FId,Page);
                 A -> A end,
...
handle_call({feed_refresh,FId,Page},From,State) ->
    Reply = store:entries_in_feed(FId,Page),
...
handle_notice(["feed", "group", GroupId, "entry", EntryId, "add"] = Route,
    ...
    case Owner == GroupId of
        false -> ok;
        true ->
            {ok, Group} = store:get(group, GroupId),
            GE = Group#group.entries_count,
            store:put(Group#group{entries_count = GE+1}),
            {ok, Subs} = store:get(group_subs, {From, GroupId}),
            SE = Subs#group_subs.user_posts_count,
            store:put(Subs#group_subs{user_posts_count = SE+1})
    end,    
...
handle_notice(["system", "tournament_tour_note"] = Route, Message, State) ->
    ...
    {ok, Tour} = store:get(tournament, TId),
...
handle_notice(["system", "tournament_ends_note"] = Route, Message, State) ->
    ...
    {TId, Results} = Message,
    {ok, Tour} = store:get(tournament, TId),
    ok = store:put(Tour#tournament{winners = Results, status = finished}),
...
handle_notice(["system", "count_user"] = Route, Message, State) ->
    case nsm_mhits:stat_word_ip_date(Message#mhits.word,Message#mhits.ip,Message#mhits.date) of
                [] -> store:put(Message#mhits{count = 1});
                DB -> DB1 = lists:nth(1,DB), store:put(Message#mhits{count = Message#mhits.count + DB1#mhits.count}) end,
...
handle_notice(["system", "create_group"] = Route, 
    ...
    CTime = erlang:now(),
    ok = store:put(#group{username = GId,
                              name = Name,
                              description = Desc,
                              publicity = Publicity,
                              creator = UId,
...
handle_notice(["db", "group", GroupId, "update_group"] = Route, 
    ...
    SaneOwner = case store:get(user, Owner) of
        {ok, _} -> Owner;
        _ -> undefined
    end,
    {ok, #group{}=Group} = store:get(group, GroupId),
        ...
        ok ->
            case Group#group.owner of
                UId -> % User is owner, transfer ownership to someone else
                    ...
                    case Members of
                        [ FirstOne | _ ] ->
                            ok = store:put(Group#group{owner = FirstOne}),
...
handle_notice(["subscription", "user", Who, "unblock_user"] = Route,
    ...
    List2 = store:list_blocked_me(Whom),
...
handle_notice(["invite", "user", UId, "add_invite_to_issuer"] = Route,
    ...
    {O} = Message,
    store:add_invite_to_issuer(UId, O),
    {noreply, State};
...
handle_notice(["transaction", "user", User, "add_transaction"] = Route,
    ...
    MP = Message,
    store:add_transaction_to_user(User,MP),
    {noreply, State};
...
add_to_group(UId, GId, Type) ->
    ...
    OK = store:put({group_subs,UId,GId,Type,0}),
    ?INFO("RES:~p",[OK]).

remove_from_group(UId, GId) ->
    store:delete(group_subs, {UId, GId}),
    {R, Group} = store:get(group, GId),
    case R of
