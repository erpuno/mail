...
map_call(NodeType,Module,Fun,Args=[ID|Rest],NodeHash0) ->
    ...
    Node = application:get_env(nsx_idgen,game_pool,5000000) div 1000000,
    ...
    ServerNode = case Node of
                      4 -> application:get_env(store,DefaultNodeAtom,'maxim@synrc.com');
                      5 -> application:get_env(store,DefaultNodeAtom,'maxim@synrc.com');
                      _ -> list_to_atom(NodeType ++ "@srv" ++ 
                           integer_to_list(NodeHash(ID)) ++
                           ".synrc.com") end,

    error_logger:info_msg("map_call: ~p",[{ServerNode,Module,Fun,Args}]),
